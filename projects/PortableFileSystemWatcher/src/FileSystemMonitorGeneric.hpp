...
class FileSystemMonitorGeneric : private boost::noncopyable
{
public:
	...
	//FileSystemMonitorGeneric(const std::string& path)
	//	: notifyFilters_(NotifyFilters::LastWrite | NotifyFilters::DirectoryName | NotifyFilters::FileName)
	//	, filter_("*.*")
	//	, implementation_( new detail::ImplementationType )
	//{
	...
	//virtual ~FileSystemMonitorGeneric()
	//{
	//}
	...
	{ 
		if ( val.size() == 0 || !utils::directoryExists(val) )
		{
			//throw std::runtime_error("InvalidDirName");
			////throw new ArgumentException(SR.GetString("InvalidDirName", new object[] { path }));
